...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  map = L.map('map').setView([12.8406, 80.1538], 13); // Default view (Chennai area)
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);

  // Add St. Joseph’s marker with a custom icon and popup
  const stJosephsIcon = L.icon({
    iconUrl: 'https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/St._Joseph%27s_College_OMR_Logo.png/800px-St._Joseph%27s_College_OMR_Logo.png',
    iconSize: [40, 40],
    iconAnchor: [20, 40],
    popupAnchor: [0, -40]
  });
  
  L.marker([12.8406, 80.1538], { icon: stJosephsIcon })
    .addTo(map)
  ...
  snapshot.forEach(doc => {
    ...
    // Use a custom icon for each bus
    const busIcon = L.icon({
      iconUrl: `https://chart.googleapis.com/chart?chst=d_map_pin_letter&chld=${busId.charAt(0)}|FF0000|000000`, // Simple letter-based icon
      iconSize: [30, 45],
      iconAnchor: [15, 45],
      popupAnchor: [0, -45]
    });

    // Create marker for the bus
    const marker = L.marker([latitude, longitude], { icon: busIcon }).addTo(map)
      .bindPopup(`<b>${busId}</b><br>${routeName}<br><b>Speed:</b> ${bus.speed} km/h`);

    // Store marker by bus ID
